...
def registrarventa(request):
    if request.method == 'POST':
        ...
        try:
            ...
            nro_doc = f"{parametro.serie}-{parametro.nroDoc}"  # Concatenar serie y nroDoc
...
def productoinfo(request):
    ...
    if producto:
        data = {
            'unidad': producto.idUnidad.descripcion,
            'precio': producto.precio
        }
        return JsonResponse(data)
...
def consultar_documento(request, document_type, document_number):
    ...
    if response.status_code == 200:
        ...
        if document_type == 'dni':
            ...
            return JsonResponse({
                'nombre_completo': data.get('data', {}).get('nombre_o_razon_social', ''),
